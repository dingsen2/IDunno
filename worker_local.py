            ...
            while True:
                ...
                if data:
                    ...
                    if msg_type == 'assign':
                        ...
                        file_path = os.path.join('local_images', file_name)
                        src_coord_host = msg['src_coord_host']
                        # return back one image's prediction
                        pred_result = self.predict_image(file_path, job_type)
                        pred_msg = {
                            'type':'pred',
                            'vm_id' : str(self.get_id_from_host(self.host)),
                            'job_type':job_type,
