...
class Server:
            ...
            while True:
                ...
                if data:
                    ...
                    #             ss.sendto(json.dumps(failed_msg).encode('utf-8'), (host, DEFAULT_PORT_SDFS))
                        ...
                        # with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sss:
                        #     failed_vm_msg = {
                        #         'type':'m_failed',
                        #         'failed_mid':str(self.get_id_from_host(msg['host']))
                        #     }
                        #     for coord_ip in CANDIDATE_COORDINATORS:
                        #         sss.sendto(json.dumps(failed_vm_msg).encode('utf-8'), (coord_ip, DEFAULT_PORT_WORKER_INPUT))
                        for f in idm[fid]:
                            ...
                            try:
                                if self.id == max(replicas):
                                    ...
                                    # also send the messages with the id of the failed vm to the two coordinators
                                    # with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
                                    #     failed_vm_msg = {
                                    #         'type':'m_failed',
                                    #         'failed_mid':str(self.get_id_from_host(msg['host']))
    ...
    def get_id_from_host(self, host):
        """
        e.g. fa22-cs425-3201.cs.illinois.edu -> 1
        :param host: host name
        :return: an integer id
        """
        return int(host.split('.')[0].split('-')[-1][-2:])
    ...
    def put_file(self, local_file_name, sdfs_file_name):
        ...
        #     prefix = 'dingsen2' + '@' + target_host
        #     p = subprocess.Popen(['sshpass', '-p', 'Yyyy19491001', 'scp', local_file_name, prefix + ':' + os.path.join(SDFS_PATH, v_file_name)])
            # os.waitpid(p.pid, 0)
    ...
    def get_file(self, sdfs_file_name, local_file_name, num_version=None):
        ...
        if not num_version:
            ...
            prefix = 'dingsen2' + '@' + self.get_host_from_id(from_id)
            print('[INFO] Get file %s from chosen replica %d' % (v_file_name, from_id))
            p = subprocess.Popen(['sshpass', '-p', 'Yyyy19491001','scp', prefix + ':' + os.path.join(SDFS_PATH, v_file_name), local_file_name])
            os.waitpid(p.pid, 0)
        # to get several updated version (command get-versions)
            ...
            with open(local_file_name, 'a') as af:
                for i in range(v - 1, v - 1 - num_version, -1):
                    ...
                    p = subprocess.Popen(['sshpass', '-p', 'Yyyy19491001','scp', prefix + ':' + os.path.join(SDFS_PATH, v_file_name), v_file_name])
                    os.waitpid(p.pid, 0)
                    af.write('\n### version %d\n' % i)
